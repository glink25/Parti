...
describe('skyward generation and contracts', () => {
  ...
  it('keeps bosses centered in the top arena band and moving horizontally', () => { const chunk = generateChunk(42, 9, 2), boss = chunk.enemies[0]!, strategy = enemyStrategies.require(boss.kind), start = strategy.position(boss, runtimeContext(contextFor(42, 9, 2), 0, 0)), later = strategy.position(boss, runtimeContext(contextFor(42, 9, 2), 0, 900)); expect(boss.x).toBe(450); expect(boss.y).toBeGreaterThanOrEqual(chunk.baseY + 1580); expect(later.x).not.toBe(start.x); expect(later.y).toBe(start.y); });
