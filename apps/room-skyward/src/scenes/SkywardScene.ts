...
import { biomeForChunk, enemyStrategies, PICKUP_PRESENTATION, platformStrategies } from '../content';
...
import { BOSS_CAMERA_ANCHOR, FLIGHT_PROFILES, applyGravity, damp, directDistance, flightSample, JUMP_SPEED, MOVE_SPEED, NORMAL_CAMERA_ANCHOR, SUPER_JUMP_SPEED, wrapX } from '../runtime/physics';
...
export class SkywardScene {
  ...
  private springContacts = new Map<string, number>();
  ...
  private simulate(dt: number) {
    ...
    const rocket = this.effect(me, 'rocket'), propeller = this.effect(me, 'propeller'), slow = this.buff(me, 'slow-fall'), flight = rocket ?? propeller;
    const forcedAt = flight?.forcedEndingAt ?? this.localFlightForcedAt ?? undefined;
    const flightState = flight ? flightSample(flight.id as 'rocket' | 'propeller', Date.now() - flight.startedAt, forcedAt == null ? undefined : forcedAt - flight.startedAt) : null;
    if (flightState?.powered) this.local.vy = flightState.speed;
    else { if (!flight) { this.localFlightForcedAt = null; this.warnedFlightBoundary = false; } this.local.vy = applyGravity(this.local.vy, dt, slow && this.local.vy < 0 ? .42 : 1); }
    this.local.y += this.local.vy * dt;
    if (flightState?.powered && this.local.vy > 0) { const boundary = ((this.state!.completedBossCount + 1) * BOSS_INTERVAL - 1) * CHUNK_HEIGHT + BOSS_TRIGGER_OFFSET; if (this.local.y >= boundary) { this.local.y = boundary; this.local.vy = 0; this.localFlightForcedAt ??= Date.now(); if (!this.warnedFlightBoundary) { this.warnedFlightBoundary = true; this.notify('Boss 空域：飞行器失去动力'); } } }
    if (!flightState?.powered && this.local.vy <= 0) {
      ...
      if (landing) {
        ...
        else { const superJump = this.buff(me, 'super-jump'); this.local.y = landing.y + PLAYER_RADIUS; this.local.vy = superJump ? Math.max(SUPER_JUMP_SPEED, result.bounceVelocity ?? JUMP_SPEED) : result.bounceVelocity ?? JUMP_SPEED; if (landing.kind === 'spring') this.springContacts.set(landing.id, performance.now()); if (superJump || landing.kind === 'spring') { this.superJumpFxUntil = performance.now() + 420; this.visualFx.push({ kind: 'jump', x: this.local.x, y: this.local.y, startedAt: performance.now(), color: landing.kind === 'spring' ? '#fff4a8' : '#f6ef76' }); } }
      }
    }
  ...
  private drawPlatform(p: Platform, fallback: string) { const s = this.toScreen(p.x, p.y), description = platformStrategies.require(p.kind).render(p, this.runtime(Number(p.id.split(':')[0])), this.state?.entities[p.id]), localBreak = this.locallyBrokenPlatforms.get(p.id); if (description.hidden) return; const contactAt=this.springContacts.get(p.id),elapsed=contactAt==null?Infinity:performance.now()-contactAt,springCompression=elapsed<90?elapsed/90:elapsed<320?1-(elapsed-90)/230:0;if(elapsed>=320)this.springContacts.delete(p.id);drawPlatformArt(mainContext, s, p.width * this.viewport.scale, 22 * this.viewport.scale, { kind: p.kind, color: description.color || fallback, warning: Boolean(description.warning), breaking: description.breaking || localBreak != null, changedAt: description.changedAt ?? localBreak, spikeRange: description.spikeRange, springCompression }, this.viewport.scale); }
  ...
  private drawHud(width: number, height: number) { if (!this.state) return; const me = this.me(), boss = this.activeBoss(); this.text(`高度 ${Math.max(0, Math.floor(this.local.y))}  Boss ${this.state.completedBossCount}`, 18, 22, 16, '#fff'); if (me) this.drawBuffCards(me); if (boss) { const ratio = boss.hp / boss.maxHp; mainContext.fillStyle = '#2a2335'; mainContext.fillRect(width / 2 - 150, 24, 300, 18); mainContext.fillStyle = '#ff5577'; mainContext.fillRect(width / 2 - 150, 24, 300 * ratio, 18); }
    ...
    const tiltSize = 48; this.drawTiltControl(width - tiltSize - 18, Math.max(82, this.viewport.y + 76), tiltSize); }
  private drawBuffCards(me: PublicPlayer) {
    const now = Date.now(), effects = Object.values(me.effects).filter((effect): effect is ActiveEffect => Boolean(effect && (effect.endsAt == null || effect.endsAt > now)));
    effects.forEach((effect, index) => {
      const info = PICKUP_PRESENTATION[effect.id], x = 18, y = 44 + index * 38, w = 190, h = 32;
      const duration = effect.endsAt == null ? 0 : Math.max(1, effect.endsAt - effect.startedAt), remaining = effect.endsAt == null ? 1 : Math.max(0, (effect.endsAt - now) / duration);
      const warning = effect.endsAt != null && remaining < .2 && Math.floor(now / 180) % 2 === 0;
      mainContext.save(); mainContext.globalAlpha = warning ? .48 : 1;
      mainContext.fillStyle = 'rgba(12,28,54,.88)'; mainContext.fillRect(x, y, w, h);
      mainContext.fillStyle = '#7eeeff'; mainContext.fillRect(x, y + h - 4, w * remaining, 4);
      let phase = effect.endsAt == null ? '1 次' : `${Math.ceil((effect.endsAt - now) / 1000)}s`;
      if (effect.id === 'rocket' || effect.id === 'propeller') {
        const forced = effect.forcedEndingAt == null ? undefined : effect.forcedEndingAt - effect.startedAt, sample = flightSample(effect.id, now - effect.startedAt, forced);
        phase = ({ accelerating: '加速', cruising: '巡航', unpowered: '失去动力', finished: '结束' } as const)[sample.phase];
      }
      this.text(`${info.name} · ${phase}`, x + 8, y + 14, 12, '#fff'); mainContext.restore();
    });
  }
  private drawLocalEffects(s: { x: number; y: number }, r: number) { const me = this.me(); if (!me) return; const c = mainContext, now = Date.now(), visualNow = performance.now(); c.save(); if (this.buff(me, 'shield')) { c.strokeStyle = '#7eeeff'; c.lineWidth = 3; c.globalAlpha = .65 + Math.sin(visualNow * .008) * .2; c.beginPath(); c.arc(s.x, s.y, r * 1.25, 0, Math.PI * 2); c.stroke(); } const shieldBreak = (visualNow - this.shieldBreakAt) / 500; if (shieldBreak >= 0 && shieldBreak < 1) { c.strokeStyle='#7eeeff'; c.globalAlpha=1-shieldBreak; c.lineWidth=3; for(let i=0;i<8;i++){const a=i/8*Math.PI*2;c.beginPath();c.moveTo(s.x+Math.cos(a)*r*1.2,s.y+Math.sin(a)*r*1.2);c.lineTo(s.x+Math.cos(a)*r*(1.2+shieldBreak),s.y+Math.sin(a)*r*(1.2+shieldBreak));c.stroke();} } const flight = this.effect(me, 'rocket') ?? this.effect(me, 'propeller'); if (flight) { const kind=flight.id as 'rocket'|'propeller', forced=flight.forcedEndingAt==null?undefined:flight.forcedEndingAt-flight.startedAt, sample=flightSample(kind,now-flight.startedAt,forced), intensity=sample.powered ? Math.max(.2,sample.speed/FLIGHT_PROFILES[kind].maxSpeed) : .08; c.globalAlpha=sample.powered?1:.35;c.fillStyle = kind==='rocket' ? '#ffb05f' : '#8ce8ff'; const length = (10+34*intensity+Math.sin(now*.025)*5) * this.viewport.scale; for(let i=-1;i<=1;i++){c.beginPath();c.moveTo(s.x+i*r*.28-r*.18,s.y+r*.65);c.lineTo(s.x+i*r*.28,s.y+r*.65+length*(1-Math.abs(i)*.18));c.lineTo(s.x+i*r*.28+r*.18,s.y+r*.65);c.fill();} } if (this.buff(me, 'super-jump') && visualNow < this.superJumpFxUntil) { const p=1-(this.superJumpFxUntil-visualNow)/420;c.strokeStyle='#f6ef76';c.globalAlpha=1-p;c.lineWidth=4;c.beginPath();c.ellipse(s.x,s.y+r*.8,r*(.5+p*1.5),r*(.15+p*.35),0,0,Math.PI*2);c.stroke(); } if (this.buff(me, 'slow-fall') && this.local.vy < 0) { c.strokeStyle='#b6d4ff'; c.globalAlpha=.65; for(let i=-1;i<=1;i++){c.beginPath();c.moveTo(s.x+i*r*.55,s.y+r*.7);c.lineTo(s.x+i*r*.75,s.y+r*1.2);c.stroke();} } c.restore(); }
