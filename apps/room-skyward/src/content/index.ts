...
platformStrategies.register(basePlatform('fragile', false, 1.4, (p) => ({ ...p, config: { breakDelayMs: 700 } }), (p, c, state) => state?.kind === 'platform' && state.phase !== 'active' ? none() : ({ effects: [runtimeEffect(p.id, { kind: 'platform', phase: 'breaking', changedAt: c.now, until: c.now + 700 })] })));
platformStrategies.register(basePlatform('recovering', false, 1.5, (p, c, i) => { const r = c.rng(`recover:${i}`); return { ...p, config: { breakDelayMs: 320, warningMs: 320, recoverMs: r.int(2800, 4200) } }; }, (p, c, state) => state?.kind === 'platform' && state.phase !== 'active' ? none() : ({ bounceVelocity: JUMP_SPEED, effects: [runtimeEffect(p.id, { kind: 'platform', phase: 'warning', changedAt: c.now, until: c.now + 320 })] })));
platformStrategies.register(basePlatform('spikes', false, 1.1, (p, c, i) => { const r = c.rng(`spike:${i}`); return { ...p, config: { spike: { start: r.float() * .35, end: .65 + r.float() * .35, periodMs: r.int(2600, 4200), warningMs: 500, activeMs: 1100, phase: r.float() } } }; }, (p, c) => spikeActive(p, c) ? ({ damageReason: '尖刺', effects: [] }) : ({ bounceVelocity: JUMP_SPEED, effects: [] })));
platformStrategies.register(basePlatform('trigger', false, .8, (p, c, i) => { const r = c.rng(`trigger:${i}`); const outputCount = r.int(1, 3); return { ...p, config: { trigger: { mode: r.pick(['permanent', 'timed', 'sequence'] as const), durationMs: r.int(6500, 10000), requiredHits: r.int(2, 3), resetMs: 4000, outputs: Array.from({ length: outputCount }, (_, n) => `${p.id}:output:${n}`) } } }; }, (p, c, state) => { const cfg = p.config!.trigger!; const previous = state?.kind === 'trigger' && (state.until == null || state.until > c.now) ? state.count : 0; const count = Math.min(cfg.requiredHits, previous + 1); const active = cfg.mode !== 'sequence' || count >= cfg.requiredHits; const until = cfg.mode === 'permanent' && active ? null : c.now + (active ? cfg.durationMs : cfg.resetMs); return { bounceVelocity: JUMP_SPEED, effects: [runtimeEffect(p.id, { kind: 'trigger', count, activatedAt: active ? c.now : 0, until }), ...cfg.outputs.map((id) => runtimeEffect(id, { kind: 'platform', phase: active ? 'active' : 'hidden', changedAt: c.now, until }))] }; }));
platformStrategies.register(basePlatform('bridge', false, 0));
platformStrategies.register(basePlatform('spring', false, 1.2, (p) => ({ ...p, config: { spring: { start: .28, end: .72, velocity: SPRING_JUMP_SPEED } } }), (p) => ({ bounceVelocity: p.config?.spring?.velocity ?? SPRING_JUMP_SPEED, effects: [] })));
...
function transitionPlatform(p: Platform, c: RuntimeContext, state?: DynamicEntityState): DynamicEntityState | null {
  if (p.kind === 'fragile' && state?.kind === 'platform') return state.phase === 'breaking' && c.now >= (state.until ?? 0) ? { ...state, phase: 'hidden', until: null } : state;
...
function boss(id: Extract<EnemyKind, 'storm-warden' | 'sky-behemoth' | 'mechanical-core'>, attackKinds: BossPhaseDefinition['attacks']): BossStrategy {
  const base = enemy(id, 0, 32, false, { kind: 'patrol', axis: 'x', range: 250, periodMs: 4600, phase: 0 }, [], true);
