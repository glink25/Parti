...
export function furthestReachable(origin:Point,target:Point,isValid:(point:Point)=>boolean,step=8){const d=distance(origin,target);if(d<=0)return{...origin};const direction=normalize(origin,target),steps=Math.ceil(d/step);let last={...origin};for(let i=1;i<=steps;i++){const along=Math.min(d,i*step),point={x:origin.x+direction.x*along,y:origin.y+direction.y*along};if(!isValid(point))break;last=point;}return last;}
export function clippedRay(origin:Point,angle:number,maxDistance:number,isValid:(point:Point)=>boolean){return furthestReachable(origin,{x:origin.x+Math.cos(angle)*maxDistance,y:origin.y+Math.sin(angle)*maxDistance},isValid);}
