...
function CategoryButtons({ selected, onToggle }: { selected: Category[]; onToggle: (category: Category) => void }) {
  return <div className="category-grid">{CATEGORIES.map((category) => {
    const meta = CATEGORY_LABELS[category];
    const active = selected.includes(category);
    return <button key={category} className={`category ${active ? 'is-active' : ''} ${category === 'nsfw' ? 'is-nsfw' : ''}`} onClick={() => onToggle(category)} aria-pressed={active}>
      <span className="category-icon">{meta.icon}</span><span><strong>{meta.name}</strong><small>{meta.note}</small></span><span className="check">✓</span>
    </button>;
  })}</div>;
}
...
function App() {
  ...
  const selectedSummary = useMemo(() => state?.selectedCategories.map((c) => CATEGORY_LABELS[c].name).join(' · ') ?? '—', [state?.selectedCategories]);

  function toggleRoomCategory(category: Category) {
    if (!state || !isHost) return;
    const next = state.selectedCategories.includes(category) ? state.selectedCategories.filter((item) => item !== category) : [...state.selectedCategories, category];
    if (next.length) void parti.action('settings:setCategories', { categories: next });
  }
  function toggleShareCategory(category: Category) {
    setShareCategories((current) => {
  ...
  function dealRound() {
    ...
    else void parti.action('round:deal');
    setHostModalOpen(false);
  }
  function dealShareCards() {
  ...
  return <main className="app-shell">
    ...
    {appMode === 'share' ? <section className="share-layout">
      ...
      <aside className="table-panel panel">
        ...
        <div className="table-actions">
          {isHost && <button className="host-button" onClick={() => setHostModalOpen(true)}>房主设置 <span>›</span></button>}
    ...
    {hostModalOpen && <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setHostModalOpen(false)}><section className="modal host-modal" role="dialog" aria-modal="true" aria-labelledby="host-title">
      <div className="modal-head"><div><p className="eyebrow">HOST CONTROLS</p><h2 id="host-title">房主发牌台</h2></div><button className="close-button" onClick={() => setHostModalOpen(false)}>×</button></div>
      <div className="mode-grid">{(Object.keys(MODE_LABELS) as DealMode[]).map((mode) => { const meta = MODE_LABELS[mode]; const active = state?.selectedMode === mode; return <button key={mode} className={`mode-card ${active ? 'is-active' : ''}`} onClick={() => void parti.action('settings:setMode', { mode })}><span>{meta.icon}</span><strong>{meta.name}</strong><small>{meta.note}</small></button>; })}</div>
      {state?.selectedMode !== 'custom' ? <><div className="section-heading category-title"><h3>选择词牌分类</h3><span>{selectedSummary}</span></div><CategoryButtons selected={state?.selectedCategories ?? []} onToggle={toggleRoomCategory} /></> : <div className="custom-words"><label>平民词<input maxLength={20} value={customCivilianWord} onChange={(e) => setCustomCivilianWord(e.target.value)} placeholder="例如：咖啡" /></label><label>卧底词<input maxLength={20} value={customUndercoverWord} onChange={(e) => setCustomUndercoverWord(e.target.value)} placeholder="例如：奶茶" /></label></div>}
      <button className="deal-button" disabled={!canDeal} onClick={dealRound}><span>{state?.round ? '重新洗牌并发牌' : '洗牌并发牌'}</span><small>{participantCount < 3 ? `还差 ${3 - participantCount} 名收牌玩家` : `${participantCount} 人收牌 · ${undercoverCount(participantCount)} 名卧底`}</small></button>
