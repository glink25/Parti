...
export const RECIPES:readonly SpellRecipe[]=[
 {id:'flame-beam',name:'生命烈焰',pattern:['fire','life'],delivery:'beam',castMs:120,channel:true,range:620,radius:24,effects:[damage('fire',7),status('burning',1,3500)]},
 {id:'meteor',name:'天降陨石',pattern:['fire','rock','rock','fire'],delivery:'targeted',castMs:850,range:900,radius:145,effects:[damage('fire',75),damage('rock',45),status('burning',2,5000)]},
 ...
 {id:'freeze-stream',name:'冻结水流',pattern:['water','ice'],delivery:'beam',castMs:160,channel:true,range:560,radius:28,effects:[damage('ice',4),status('wet'),status('frozen',1,1800)]},
 ...
 {id:'lava-shot',name:'熔岩弹',pattern:['rock','fire'],delivery:'projectile',castMs:260,range:720,radius:48,effects:[damage('rock',28),damage('fire',22),status('burning')]},
 ...
 {id:'shatter-shot',name:'碎冰岩弹',pattern:['rock','ice'],delivery:'projectile',castMs:260,range:700,radius:42,effects:[damage('rock',34),damage('ice',16),status('chilled')]},
 {id:'storm-chain',name:'导电水链',pattern:['water','lightning'],delivery:'targeted',castMs:220,range:700,radius:150,effects:[damage('lightning',34),status('wet'),status('shocked',1,1200)]},
 {id:'flamethrower',name:'烈焰喷射',pattern:['fire','fire'],delivery:'cone',castMs:120,channel:true,range:390,radius:90,effects:[damage('fire',10),status('burning',2)]},
...
const bases:Record<Element,Omit<SpellPlan,'id'|'elements'>>={
 rock:{name:'岩弹',delivery:'projectile',castMs:220,channel:false,range:680,radius:34,effects:[damage('rock',32),{type:'impulse',amount:80}]},
 fire:{name:'火焰',delivery:'cone',castMs:100,channel:false,range:300,radius:75,effects:[damage('fire',24),status('burning')]},
 ice:{name:'冰弹',delivery:'projectile',castMs:160,channel:false,range:650,radius:30,effects:[damage('ice',22),status('chilled')]},
 life:{name:'生命术',delivery:'targeted',castMs:300,channel:false,range:480,radius:55,effects:[{type:'heal',amount:30}]},
 lightning:{name:'雷击',delivery:'targeted',castMs:80,channel:false,range:620,radius:85,effects:[damage('lightning',30),status('shocked',1,900)]},
 water:{name:'水流',delivery:'beam',castMs:80,channel:true,range:500,radius:30,effects:[damage('water',3),status('wet'),{type:'impulse',amount:45}]},
 shield:{name:'个人护盾',delivery:'self',castMs:120,channel:false,range:0,radius:0,effects:[status('shielded',1,7000)]}
};
...
export function resolveSpell(elements:readonly Element[],staff?:StaffModifier):SpellPlan|null{
 ...
 let plan:SpellPlan=recipe?{...recipe,elements:[...elements],channel:Boolean(recipe.channel)}:{id:`mixed:${elements.join('-')}`,elements:[...elements],...bases[elements[0]! ]};
