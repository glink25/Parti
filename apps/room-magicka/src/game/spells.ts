...
export function resolvePlayerSpell(elements:readonly Element[],inventory:readonly InventoryItem[],cooldowns:Readonly<Record<string,number>>,now=Date.now(),staff?:StaffModifier):SpellPlan|null{
 ...
 if(scroll&&(cooldowns[scroll.spellId]??0)<=now){const blackHole=scroll.spellId==='scroll-black-hole',effect:SpellEffect=blackHole?{type:'environment',kind:'black-hole',durationMs:8000,radius:340}:scroll.spellId==='scroll-supernova'?{type:'current-health-damage',ratio:.5}:scroll.spellId==='scroll-equilibrium'?{type:'set-health',ratio:.5}:{type:'current-health-damage',ratio:.99,bossRatio:.5};const targeted=blackHole||scroll.spellId==='scroll-annihilation',plan:SpellPlan={id:scroll.spellId,name:scroll.name,elements:[...elements],delivery:targeted?'targeted':'global',castMs:650,channel:false,range:targeted?700:0,radius:blackHole?340:scroll.spellId==='scroll-annihilation'?75:0,effects:[effect],pierce:0,tickMs:150,recoveryMs:240,projectileSpeed:0,coneAngle:.9,traits:[]};plan.presentation=presentationFor(plan);return staff?.matches(plan)?staff.transform(plan):plan;}
