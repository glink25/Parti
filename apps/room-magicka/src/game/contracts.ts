export const SCHEMA_VERSION=5, WORLD_WIDTH=4800, WORLD_HEIGHT=3360, HUD_HEIGHT=118, ELEMENT_SLOTS=4;
...
export type GameState={schemaVersion:number;phase:Phase;hostId:string|null;startedAt:number|null;runSeed:number;stageIndex:number;stage:StageProgress|null;exploredRoomIds:string[];players:Record<string,PublicPlayer>;enemies:Record<string,EnemyState>;projectiles:Record<string,HostProjectile>;environments:Record<string,EnvironmentState>;encounters:Record<string,RegionEncounter>;worldSequence:number;totalKills:number;message:string};
...
export type HostWorldReport={sequence:number;enemies:EnemyState[];projectiles:HostProjectile[];environments:EnvironmentState[];encounters:RegionEncounter[];stage:StageProgress};
