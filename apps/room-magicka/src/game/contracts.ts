...
export type SpellVisual={id:string;kind:'cone'|'impact'|'beam'|'projectile'|'heal';origin:Point;target:Point;radius:number;angle:number;color:Element;startedAt:number;endsAt:number};
export type SharedSpellVisual={eventId:string;sourcePlayerId:string;kind:SpellVisual['kind'];origin:Point;target:Point;radius:number;angle:number;color:Element;durationMs:number};
