...
function partiWorkerBundle(outDir: string): Plugin {
  ...
  return {
    ...
    buildStart() {
      ...
      for (const file of listFiles(path.resolve(appDir, 'src/game'))) this.addWatchFile(file);
    },
    async closeBundle() {
...
export default defineConfig(({ mode }) => {
  ...
  if (mode === 'room-dev' && !roomDevOutDir) throw new Error('PARTI_ROOM_DEV_OUT_DIR is required');
  if (mode === 'room-build' && !roomBuildOutDir) throw new Error('PARTI_ROOM_BUILD_OUT_DIR is required');
