...
type Stage = 'waiting' | 'role-check' | 'cupid' | 'guard' | 'werewolf' | 'witch' | 'seer' |
  ...
  'badge-transfer' | 'day-vote' | 'finished';
...
function sendSecrets(ctx: Context, id: string) {
  ...
  if (card.role === 'werewolf') ctx.send(id, 'werewolf:pack', { round: ctx.state.round, playerIds: Object.keys(cards).filter((pid) => cards[pid].role === 'werewolf') });
...
function advance(ctx: Context) {
  ...
  if (stage === 'hunter') {
    ...
    else if (ctx.state.lastVotes.length) beginNight(ctx); else { setStage(ctx, 'day-vote'); ctx.state.voteCandidates = aliveIds(ctx); voteRoundCandidates = [...ctx.state.voteCandidates]; }
    return;
  }
...
export default defineRoom({
  ...
  onRestore(ctx: Context) { cards = {}; lovers = []; submissions = {}; ctx.state.stage = 'waiting'; ctx.state.dealtPlayerIds = []; ctx.state.deadPlayerIds = []; ctx.state.deaths = []; ctx.state.result = null; ctx.state.revealedRoles = null; ctx.state.notice = '房间已恢复，秘密状态无法恢复，请重新发牌'; syncPlayers(ctx); },
  actions: {
    ...
    'game:start'(ctx: Context, event: { player: Player }) {
      ...
      cards = dealRoles(ids, ctx.state.configuredRoles, () => ctx.random()); lovers = []; guardedLastNight = null; antidoteAvailable = true; poisonAvailable = true; pendingDeaths = []; hunterShotUsed = false;
      ctx.state.round += 1; ctx.state.day = 0; ctx.state.dealtPlayerIds = ids; ctx.state.deadPlayerIds = []; ctx.state.deaths = []; ctx.state.sheriffId = null; ctx.state.sheriffCandidates = []; ctx.state.lastVotes = []; ctx.state.lastDeaths = []; ctx.state.result = null; ctx.state.revealedRoles = null; ctx.state.notice = null;
    ...
    'game:restart'(ctx: Context, event: { player: Player }) {
      if (event.player.id !== ctx.host.id) return;
      cards = {}; lovers = []; submissions = {}; ctx.state.stage = 'waiting'; ctx.state.dealtPlayerIds = []; ctx.state.deadPlayerIds = []; ctx.state.deaths = []; ctx.state.result = null; ctx.state.revealedRoles = null; ctx.state.sheriffId = null; ctx.state.notice = '本局已结束，可重新配置并发牌';
    },
    'stage:submit'(ctx: Context, event: { player: Player; payload: unknown }) {
      if (ctx.state.stage === 'waiting' || ctx.state.stage === 'finished' || ctx.state.stage === 'badge-transfer') return;
    ...
    'sheriff:badge'(ctx: Context, event: { player: Player; payload: unknown }) {
      ...
      if (ctx.state.lastVotes.length) beginNight(ctx); else { setStage(ctx, 'day-vote'); ctx.state.voteCandidates = aliveIds(ctx); voteRoundCandidates = [...ctx.state.voteCandidates]; }
    },
  },
});
