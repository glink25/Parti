...
type Bullet = { id: string; x: number; y: number; vy: number; life: number; cosmetic: boolean };
...
export class SkywardScene {
  ...
  private simulate(dt: number, now: number) {
    ...
    const rocket = this.buff(me, 'rocket'), propeller = this.buff(me, 'propeller'), slow = this.buff(me, 'slow-fall');
    if (rocket) this.local.vy = 760; else if (propeller) this.local.vy = 520; else { this.local.vy += (slow && this.local.vy < 0 ? GRAVITY * .42 : GRAVITY) * dt; }
    ...
    if (!rocket && !propeller && this.local.vy <= 0) {
      ...
      if (landing) {
        void parti.action('landPlatform', { platformId: landing.id });
        if (landing.kind === 'fragile') { this.notify('脆弱平台碎裂'); }
        else if (landing.kind === 'spikes') this.die('尖刺');
  ...
  private simulateBullets(dt: number) { for (const b of this.bullets) { b.y += b.vy * dt; b.life -= dt; } this.bullets = this.bullets.filter((b) => b.life > 0); for (const b of this.bullets) { if (b.cosmetic) continue; const enemy = this.enemies().find((e) => directDistance(b.x, e.x) < e.radius + 12 && Math.abs(b.y - e.y) < e.radius + 18); if (enemy) { b.life = 0; const sequence = ++this.hitSequence; void parti.action('hitEnemy', { eventId: `${parti.playerId}:hit:${sequence}`, sequence, shotId: b.id, enemyId: enemy.id, damage: this.buff(this.me()!, 'power') ? 2 : 1 }); } } }
  ...
  private drawPlatform(p: Platform, color: string) { const c = mainContext, s = this.toScreen(p.x, p.y), w = p.width * this.viewport.scale, h = 20 * this.viewport.scale; const colors: Record<string, string> = { moving: '#59c3ff', fragile: '#d5b895', recovering: '#c57bff', spikes: '#ff5577', trigger: '#46e0b3', bridge: '#46e0b3', 'boss-exit': '#ffd35a' }; c.fillStyle = colors[p.kind] ?? color; c.fillRect(s.x - w / 2, s.y - h / 2, w, h); if (p.kind === 'spikes') { c.fillStyle = '#fff'; for (let x = s.x - w / 2; x < s.x + w / 2; x += 18 * this.viewport.scale) { c.beginPath(); c.moveTo(x, s.y - h / 2); c.lineTo(x + 9 * this.viewport.scale, s.y - h / 2 - 18 * this.viewport.scale); c.lineTo(x + 18 * this.viewport.scale, s.y - h / 2); c.fill(); } } this.label(p.kind, s.x, s.y + 13 * this.viewport.scale); }
