...
export type EncounterState = { id: string; roomId: string; kind: 'roaming' | 'sealed'; status: 'dormant' | 'active' | 'cleared'; monsterIds: string[]; waves: string[][]; currentWave: number; totalWaves: number; nextWaveAt: number | null; scaledForPlayers: number | null; activatedAt: number | null; clearedAt: number | null };
...
export type PlayerState = Combatant & { id: string; name: string; connected: boolean; alive: boolean; position: Vec2; aim: Vec2; selectedElements: Element[]; forceVelocity?: Vec2; positionEpoch: number; lastPoseSequence: number; lastCastSequence: number; gold: number; inventory: InventoryItem[]; equipment: PlayerEquipment; scrollCooldowns: Partial<Record<ScrollId, number>> };
