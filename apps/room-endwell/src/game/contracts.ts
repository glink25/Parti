...
export type InstantSpec = { kind: 'resurrect' | 'teleport' | 'cleanse' | 'haste' | 'supernova' | 'equilibrium' | 'annihilation'; radius?: number; durationMs?: number; amount?: number };
...
export type EntityState = Combatant & { id: string; kind: EntityKind; archetype?: SummonArchetype | 'meteor-warning' | 'meteor-impact'; ownerId?: string; position: Vec2; velocity?: Vec2; forceVelocity?: Vec2; direction?: Vec2; radius: number; createdAt: number; expiresAt: number | null; detached: boolean; source?: { spell: SpellSpec; tickMs: number; nextTickAt: number }; obstacle?: { blocksMovement: boolean; blocksProjectile: boolean; blocksBeam: boolean; blocksSpray: boolean; material: 'stone' | 'ice' | 'magic' | 'flesh' } };
