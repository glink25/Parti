...
describe('deterministic roguelike blueprint', () => {
  ...
  it('gives every non-spawn, non-boss room a fixed three-wave encounter', () => { const stage = generateStage(77, 0), encounterRooms = new Set(stage.encounters.map((encounter) => encounter.roomId)); expect(stage.encounters.map((encounter) => encounter.waves.map((wave) => wave.length))).toEqual(stage.encounters.map(() => [4, 6, 8])); for (const room of stage.world.rooms) if (room.id !== 'room-0' && room.id !== stage.boss.roomId) expect(encounterRooms.has(room.id)).toBe(true); for (const encounter of stage.encounters) { expect(encounter.totalWaves).toBe(3); expect(encounter.currentWave).toBe(0); expect(encounter.scaledForPlayers).toBeNull(); expect(new Set(encounter.waves.flat())).toEqual(new Set(encounter.monsterIds)); } });
  it('keeps merchant and forge encounters dormant instead of pre-cleared', () => { const stage = generateStage(91, 0), merchantRoom = roomAt(stage, stage.merchant.position)!.id, forgeRoom = roomAt(stage, stage.forge.position)!.id; expect(stage.encounters.find((encounter) => encounter.roomId === merchantRoom)?.status).toBe('dormant'); expect(stage.encounters.find((encounter) => encounter.roomId === forgeRoom)?.status).toBe('dormant'); });
});
