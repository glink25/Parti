...
export function applyHit(state: GameState, hit: HitEvent, now: number): HitResolution {
  ...
  for (const effect of hit.payload.effects ?? []) {
    if (effect.type === 'interrupt' && target.cast.phase !== 'idle') target.cast = { ...target.cast, phase: 'interrupted', phaseEndsAt: now + 350 };
    else if (effect.type === 'knockback' && sourceEntity) applyForce(state, sourceEntity, owner, hit.targetId, effect.force, now);
  }
  for (const [key, amount] of Object.entries(hit.payload.statusBuildup ?? {})) { const type = key as ElementStatusType; if (immune(target, type)) { target.buildup[type] = 0; continue; } target.buildup[type] = (target.buildup[type] ?? 0) + (amount ?? 0); if ((target.buildup[type] ?? 0) >= STATUS_THRESHOLD) { target.buildup[type] = 0; status(target, type, now, hit.ownerId, type === 'shocked' ? 1200 : type === 'chilled' ? 3000 : 5000); events.push({ type: 'status_applied', targetId: hit.targetId, status: type }); } }
