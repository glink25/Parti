...
describe('generated spell resolver', () => {
  ...
  it('uses gentle per-tick force for water beams', () => { expect(resolveSpell(['water', 'fire', 'life']).payload.effects).toContainEqual({ type: 'knockback', force: 80 }); expect(resolveSpell(['water', 'ice']).payload.effects).toContainEqual({ type: 'knockback', force: 70 }); });
});
