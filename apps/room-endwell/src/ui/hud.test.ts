...
describe('mobile HUD layout', () => {
  ...
  it('leaves the transparent element and message regions open to world input', () => { const layout = computeHudLayout(390, 844), point = layout.elementCenters[3]!, message = { x: layout.message.x + layout.message.w / 2, y: layout.message.y + layout.message.h / 2 }; expect(hudBlocksWorld(layout, point)).toBe(false); expect(hudBlocksWorld(layout, message)).toBe(false); expect(hudBlocksWorld(layout, { x: 195, y: 300 })).toBe(false); });
