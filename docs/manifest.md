...
ZIP、GitHub 导入和内置房间会收集 package 根目录中的全部文件。GitHub 导入接受仓库根地址、
`tree/<ref>/<folder>` 文件夹地址或 `blob/<ref>/<file>` 文件地址；tree/blob 会把搜索范围限制在
对应目录，并与市场预检使用同一套“合法 manifest + 全部声明入口”候选规则。Worker 目前仍为
单入口源码，不支持相对模块 import。
